...
def sign_up(request):
    if request.method == 'POST':
        ...
        reg.save() 
        regstatus = firmregistrationstatusmodel(status="Received_Registration",Received_Registration=formatted_datetime,reg_id=reg.id,email=reg.email)
        regstatusdb=regstatus.save()
