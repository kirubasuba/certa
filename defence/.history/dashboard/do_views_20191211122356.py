...
def rowselect(request,id):
    ...
    taf=TAapplicationfiles.objects.filter(user_id=request.id,refid=idprefix).order_by('refpath')
    idg=idgenerationmodel.objects.filter(user_id=request.id,idprefix=idprefix).first()
    print(taf,'taf')
    print(idg,'idg')
