...
from .models import TAapplicationmodel,proforma_A_model,checklistmodel,idgenerationmodel,TAapplicationfiles,commentsmodel
...
class commentsUploadForm(forms.ModelForm):
    ...
    name = forms.CharField(label='Anexture name', widget=forms.TextInput(attrs={'cols': 80, 'rows': 2}))
    ...
    action_taken = forms.CharField(label='action_taken', widget=forms.TextInput(attrs={'cols': 80, 'rows': 2}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(commentsUploadForm,self).__init__(*args,**kwargs)
        for visible in self.visible_fields():
            visible.field.widget.attrs['class'] = 'form-control form-control-user'