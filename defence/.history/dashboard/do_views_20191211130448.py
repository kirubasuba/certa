...
def rowselect(request,id):
    ...
    taff=TAapplicationfiles.objects.filter(user_id=id,refid=idprefix).values('refpath').order_by('refpath')
    ...
    formatted_datetime = date_joined.strftime("%Y-%m-%d")
    role=request.role
    print(taff,formatted_datetime,role,idprefix,id,'role')
    # comments = commentsmodel(name=taff,commented_date=formatted_datetime,commented_by=role,idprefix=idprefix,user_id=id)
