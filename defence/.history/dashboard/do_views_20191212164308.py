...
def rowselect(request,id):
    ...
    taf=TAapplicationfiles.objects.filter(user_id=id,refid=idprefix).values('refpath').order_by('refpath').first()
    ...
    print(taf.refpath,'taf')
    
    for anex_name in taf
        anexture_name = taf.refpath
        print(anexture_name,'hello')
