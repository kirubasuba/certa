...
def rowselect(request,id):
    ...
    formatted_datetime = date_joined.strftime("%Y-%m-%d")
    role=request.role
    print(formatted_datetime,role,idprefix,id,'role')
    for anex_name in get_refpath:
        anexturename=[anex_name['refpath']] 
        comments = commentsmodel(name=anex_name.anexturename,commented_date=formatted_datetime,commented_by=role,idprefix=idprefix,user_id=id)
    print(anexturename,'taffq')
        # comments = commentsmodel(name=anex_name.refpath,commented_date=formatted_datetime,commented_by=role,idprefix=idprefix,user_id=id)
    ...
    return render(request, 'rd/comments_view_doc.html',{'form':form})
