...
def rowselect(request,id):
    ...
    taf=TAapplicationfiles.objects.filter(user_id=id,refid=idprefix).order_by('refpath')
...
def addcomment(request):
    ...
    print(idprefix,anexture_name,userid,'refid')

    # idg=idgenerationmodel.objects.filter(idprefix=idprefix).first()
    # print("rcma",idg.rcma)
    # get_username=Group.objects.filter(VG=idg.rcma).first()
    role=request.role
    date_joined = datetime.now()
    formatted_datetime = date_joined.strftime("%Y-%m-%d")

    comments = commentsmodel(name=anexture_name,comments=comments,commented_date=formatted_datetime,commented_by=role,responsible=responsible,idprefix=idprefix,status=status,user_id=userid)
    comments.save()

    print(anexture_name,comments,formatted_datetime,role,responsible,idprefix,status,'details')
