...
from .models import TAapplicationmodel,proforma_A_model,TAapplicationfiles,statusmodel,commentsmodel
...
def rowselect(request,id):
    ...
    taa=TAapplicationmodel.objects.filter(user_id=id)
    print(id,taa.idprefix,'hello')
    # idg=idgenerationmodel.objects.filter(user_id=request.user.id).first()
    # idprefix=request.POST['idpre']  
    # print('idprefixxxxxxxxxxxxxx')
    # print(idprefix,'idprefix')
