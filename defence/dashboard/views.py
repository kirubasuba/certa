...
def dashboard(request):
    ...
    # count_notifify=commentsmodel.objects.filter(responsible=current_user) | commentsmodel.objects.filter(commented_by=current_user,isAction="True")
    count_notifify=commentsmodel.objects.filter(Q(responsible=current_user) | Q(commented_by=current_user,isAction="True",notification_alert="temp_close"))
    get_count=count_notifify.count()
    get_comments=commentsmodel.objects.filter(Q(responsible=current_user) | Q(commented_by=current_user,isAction="True")).values('name','comments','anexture_id','isAction','notification_alert')
    print(count_notifify,get_comments,get_count,'first') 
    context={
        'count':get_count,
        'show_comment' : get_comments
        
    }
   
    if request.VG is None:
        if request.role=='TAApplicant':
            ...
            status_to_get ={"Recommended","Send_to_TCS_TAC","Send_to_TCS_DO","Draft_TA","Forward_to_GD","Forward_to_CE","Issued_TA"}            
...
def password_reset(request):
    if request.method == 'POST':
        ...
        temppass=request.POST['tempass']
        pass1=request.POST['pass1']
        pass2=request.POST['pass2']
        print('testedddddddd',pass1)
        user = authenticate(username=username, password=temppass)
        if user is not None:
            if user.is_active:
                ...
                # sender = 'cemilac.drdo@gmail.com'
                # receivers = [email]

                # message = """From:<cemilac.drdo@gmail.com>
                # Subject: Password Reset Successfully!!!

                # Hi""" +username+""",Your Password Reset Successfully.
                # Now you can login with the following credentials.
                # Username: """+username+""", Passwords: """+pass1
                ...
                send_mail('Password Reset Successfully!!!', 'Hi '+username+',Your Password Reset Successfully.Now you can login with the following credentials.Username: '+username+', Password: '+pass1, 'cemilac.drdo@gmail.com', [email,])
...
def hold_remarks(request):
    ...
    get_taap_id=firmregistrationstatusmodel.objects.filter(email=reg.email).first()
    if get_taap_id:
        get_taap_id.status='Hold_Registration'
        get_taap_id.Hold_Registration=formatted_datetime
        get_taap_id.save()
    else:
        get_reg=firmregistrationstatusmodel(status='Hold_Registration',Hold_Registration=formatted_datetime,reg_id=idnew)
        # get_reg.status='Hold_Registration'
        # get_reg.Hold_Registration=formatted_datetime
        get_reg.save()
...
def new_user(request):
    ...
    if request.method == 'POST':
        ...
        # reg.status='active'
        # reg.save()
        # grp=Group.objects.filter(name='TAApplicant').first()
        ...
        # Hi""" +username+""",Your account is Successfully Registered in CEMILAC.
        # Now you can login with the following credentials.
        # Username: """+username+""", Temporary Passwords: """+password
        ...
        send_mail('Registered Successfully!!!', 'Hi '+username+',Your account is Successfully Registered in CEMILAC.Now you can login with the following credentials.Username: '+username+', Temporary Password:'+password, 'cemilac.drdo@gmail.com', [email,])
...
@login_required(login_url=settings.LOGIN_URL)
@role_required(allowed_roles=["AppAdmin"])
def new_cemilac_user(request):
    print(request.role,'lllllllllllllllllllllllllllllllll')
    if request.method == 'POST':
...
@login_required(login_url=settings.LOGIN_URL)
@role_required(allowed_roles=["TAApplicant"])
...
def generatepdf(request):
    if request.POST:
        ...
        if form.is_valid():
            ...
            # unique_filename = str(uuid.uuid4())
            filename = fs.save("marksheet.pdf", myfile)
            uploaded_file_url = fs.path(filename)
            print(uploaded_file_url,'url')
...
@login_required(login_url=settings.LOGIN_URL)
@role_required(allowed_roles=["TAApplicant","RD","TA Coordinator","Dealing Officer","TCS-CE","TCS-GD",
...
def dashboard_status(request):
    ...
    # print(get_rcma.rcma,'get_rcma')
    get_vg_id=Group.objects.filter(VG=request.VG).values('name')
    print(get_vg_id,'get_vg')
    get_user=User.objects.filter(groups__name__in=[get_vg_id]).values('id','first_name').exclude(groups__name__endswith='RD').order_by('groups__name')
    print(get_user,'get_user_id')
    ...
    return render(request, 'dashboard/dashboard_count.html',{'firmdetails': formid,'get_user':get_user})
...
def notification(request):
    print("inside notifiation")
    anextureID=request.GET['anexture_id']
    print("inside",anextureID) 
    current_user=request.user.username
    get_data=commentsmodel.objects.filter(anexture_id=anextureID)
    get_data11=commentsmodel.objects.filter(anexture_id=anextureID).values('isAction').first()
    print("get_data",get_data11) 
    context={
        'details':get_data
    }
    # print("inside notifiation",current_user,get_data) 
    ...
    if get_data11=='false':
        ...
        print("inside notifiation",current_user,get_data) 
        return render(request, 'dashboard/dashboard_response.html',context)